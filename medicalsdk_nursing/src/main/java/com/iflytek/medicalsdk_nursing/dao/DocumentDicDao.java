...
public class DocumentDicDao {
    ...
    public boolean saveOrUpdateDocumentDicList(List<DocumentDic> list) {
        ...
        try {
            ...
            sql.append("(nmrID,nmrName,workID,interfaceName)");
            sql.append("values(?,?,?,?)");
            ...
            for (DocumentDic info : list) {
                ...
                stat.bindString(3, StringUtils.nullStrToEmpty(info.getWorkID()));
                stat.bindString(4, StringUtils.nullStrToEmpty(info.getInterfaceName()));
